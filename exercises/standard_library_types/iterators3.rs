...
pub fn divide(mut a: i32, b: i32) -> Result<i32, DivisionError> {
    if a == 0 {
        return Ok(0);
    }
    ...
    loop {
        ...
        if a < 0 {
            break Err(DivisionError::NotDivisible(NotDivisibleError {
                dividend: dividend,
                divisor: b,
            }));
        }
...
mod tests {
    ...
    fn result_with_list() {
        ...
        let x: Result<Vec<i32>, DivisionError> =
            Ok(division_results.filter_map(|result| result.ok()).collect());
